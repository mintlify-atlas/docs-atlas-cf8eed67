...
description: Shell integration, key bindings, and completion for Zsh
---

fzf provides comprehensive integration with Zsh including key bindings, fuzzy completion, and customization options.

## Setup

<ParamField path="fzf --zsh" type="command">
...
fzf provides three default key bindings for Zsh:
...
<ParamField path="CTRL-R" type="keybinding">
  ...
  - Supports Perl for better multi-line handling
...
fzf provides fuzzy completion for various commands using the `**` trigger with the TAB key.
...
<ParamField path="__fzf_list_hosts" type="function">
  Override the default host list function
  
  ```bash
  __fzf_list_hosts() {
    # Return custom host list, one per line
    cat ~/my-hosts.txt | sort -u
  }
  ```
</ParamField>
...
<ParamField path="_fzf_complete_COMMAND" type="function">
  Custom fuzzy completion function (automatically picked up by Zsh)
  ...
  <Note>
    Unlike Bash, Zsh automatically picks up the completion function by naming convention.
  </Note>
</ParamField>
...
```bash
# Different completion based on previous argument
_fzf_complete_git() {
  local tokens
  tokens=(${(z)1})
...
Key bindings work in both Emacs and Vi mode:

```bash
# Vi mode
bindkey -v

# fzf key bindings work in both insert and command mode
# CTRL-T, CTRL-R, ALT-C are available in both modes
```

### History Options

Zsh's history integration is more advanced:

- Supports `zsh/parameter` module for better performance
- Perl-based multi-line history handling
- Associative history array access
- Better handling of history timestamps
